define([
    "app",
    "text!bundle/producer/view/brands/container.html"
], function(
    App,
    containerTpl
){
    return App.module("Producer.Brands", function (Brands, App, Backbone, Marionette, $, _) {
		
		var Model = Backbone.Model.extend();
		
		var Collection = App.PageableCollection.extend({
			url: 'mock/producer/brands.json'
		});
		
		this.Container = Marionette.LayoutView.extend({
			template: _.template(containerTpl),
			regions: {
				list: "#producer-brands-list",
				pagination: "#producer-brands-pagination"
			},
			initialize: function() {
				var self = this;
